...
function JobSection({ title, items }: { title: string; items: string[] }) {
  ...
  const labelWidth = language === 'en' ? 'min-w-[200px]' : 'min-w-[80px]';

  return (
    <div className='flex gap-[30px] items-start w-full max-w-[600px]'>
      <div className={`${labelWidth} flex-shrink-0 flex justify-start`}>
        <h4 className='font-bold text-base md:text-lg text-black leading-[22px] text-left whitespace-pre-line'>
          {title}
        </h4>
      </div>
    ...
    <section id='CareersJobList' className='w-full relative py-20 px-4'>
      <div className='max-w-[1280px] mx-auto flex flex-col items-center gap-12'>
        ...
        <div className='flex flex-col gap-[30px] items-center text-center max-w-[907px] py-10'>
          <h2 className='font-bold text-heading-base sm:text-heading-sm md:text-heading-md leading-tight whitespace-pre-line'>
